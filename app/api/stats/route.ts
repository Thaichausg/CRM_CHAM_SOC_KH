...
import { db } from '@/db';
import { customers, customerProfiles, careTasks, pipelineItems, scripts } from '@/db/schema';
...
export async function GET() {
  ...
  const activeContracts = allCustomers.filter(c => (c.status || '').toLowerCase().includes('đang')).length;
  const lapsedContracts = allCustomers.filter(c => (c.status || '').toLowerCase().includes('mất')).length;
  ...
  const vipCustomers = allProfiles.filter(p => (p.rank || '').includes('Vàng') || (p.rank || '').includes('Kim cương')).length;
